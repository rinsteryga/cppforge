...
namespace cppforge::services
{
    ...
    void TaskManager::setTaskRepository(cppforge::repositories::ICodingTaskRepository *repo)
    {
        m_taskRepo = repo;
    }
    ...
    cppforge::entities::CodingTask TaskManager::getRandomDuelTaskFromDb()
    {
        if (m_taskRepo)
        {
            auto taskOpt = m_taskRepo->getRandomDuelTask();
            if (taskOpt)
            {
        ...
        qDebug() << "No duel tasks found in database (or repository not set)!";
        return {};
    }
} // namespace cppforge::services
