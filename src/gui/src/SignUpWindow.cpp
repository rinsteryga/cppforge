...
SignUpWindow::SignUpWindow(QWidget *parent)
    : QWidget(parent, Qt::Window), passwordVisible_(false) {
...
void SignUpWindow::setupInputFields() {
  ...
  if (!openEyePixmap.isNull()) {
    openEyePixmap = openEyePixmap.scaled(20, 20, Qt::KeepAspectRatio,
                                         Qt::SmoothTransformation);
...
void SignUpWindow::setupBackToLoginLink() {
  backToLoginButton_ =
      std::make_unique<QPushButton>("Already have an account? Sign in");
...
void SignUpWindow::setupLayout() {
  ...
  centerContainer->setSizePolicy(QSizePolicy::Expanding,
