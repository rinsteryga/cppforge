...
TaskWindow::TaskWindow(QWidget *parent)
    ...
    , compiler_(new QProcess(this))
    , currentModuleId_(-1)
    , currentTaskId_(-1)
    , currentTheoryIndex_(-1)
    , tabWidget_(nullptr)
    , theoryTab_(nullptr)
    , practiceTab_(nullptr)
    , theorySelector_(nullptr)
    , theoryDisplay_(nullptr)
    , prevTheoryButton_(nullptr)
    , nextTheoryButton_(nullptr)
    , theoryPageLabel_(nullptr)
    , practiceSplitter_(nullptr)
    , taskPanel_(nullptr)
    , taskTitle_(nullptr)
    , taskDescription_(nullptr)
    , taskSelector_(nullptr)
    , codeEditor_(nullptr)
    , inputField_(nullptr)
    , outputField_(nullptr)
    , runButton_(nullptr)
    , checkButton_(nullptr)
...
TaskWindow::~TaskWindow() {
    saveCurrentCode();
    if (compiler_) compiler_->deleteLater();
}
...
void TaskWindow::setupConnections() {
    ...
    connect(nextTheoryButton_, &QPushButton::clicked, this, [this]() {
    ...
    connect(compiler_, &QProcess::readyReadStandardOutput,
            this, &TaskWindow::onProcessOutput);
    connect(compiler_, &QProcess::readyReadStandardError,
            this, &TaskWindow::onProcessOutput);
}
...
void TaskWindow::updateTheoryContent() {
    if (currentTheoryIndex_ < 0 || currentTheoryIndex_ >= theoryIds_.size()) return;
    ...
    for (int i = 0; i < task.testCases.size(); ++i) {
        ...
        if (output == task.testCases[i].expectedOutput) {
            ...
            results += QString("❌ Тест %1: Ожидалось '%2', получено '%3'\n")
                          .arg(i + 1).arg(task.testCases[i].expectedOutput).arg(output);
        }
...
int TaskWindow::calculateModuleProgress(int moduleId) {
    ...
    Module module = taskManager_->getModule(moduleId);
    if (!module.isValid() || module.tasks.isEmpty()) return 0;
