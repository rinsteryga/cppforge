...
В рамках нашей платформы мы предполагаем, что вы работаете в Linux-подобной среде или используете подсистему WSL (Windows Subsystem for Linux). Не пугайтесь терминала — скоро интерфейс командной строки (CLI) станет вашим основным профессиональным инструментом.', 2);
...
Рассмотрим несколько базовых утилит (команд) для навигации по файловой системе:
  ...
  touch — утилита, предназначенная для обновления временных меток файла, но если файла не существует, она создаст новый пустой файл: touch main.c.', 3)
...
И не забывайте: точка с запятой ; в C — это терминатор инструкции, она обозначает логический конец выражения.', 4)
    RETURNING id INTO v_lesson_id;
...
Команда gcc main.c -o program выполнит все 4 этапа автоматически.', 5)
    ...
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
    (v_mod_intro, '1.6. Первый вывод - функция printf', 'Программы без ввода/вывода (I/O, Input/Output) имеют мало практической ценности. Когда ОС запускает вашу программу, она автоматически открывает три стандартных файловых дескриптора: stdin, stdout, stderr.
