...
(v_mod_id, '13.1. Smart-pointers (Умные указатели)', $content$Ручное управление памятью через `new` и `delete` — это главный источник багов (утечки, двойное освобождение). В современном C++ мы используем умные указатели из `<memory>`:
...
3. **`std::weak_ptr<T>`**: "Слабая" ссылка на объект, которым владеет `shared_ptr`. Не увеличивает счетчик. Нужен для разрыва циклических зависимостей.$content$, 1)
    ...
    VALUES (
        v_lesson_id, 
        'Умный указатель', 
        $description$Напишите функцию `int get_val(std::unique_ptr<int>& ptr)`, которая возвращает значение по указателю, если он не пуст, иначе возвращает 0. Функция `main` уже написана.
...
Вывод: 42$description$, 
    ...
    -- 13.2
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '13.2. RAII — основа безопасности', $content$RAII (Resource Acquisition Is Initialization) — это концепция, на которой держится весь C++. 
...
Примеры RAII в стандарте: `std::vector`, `std::string`, `std::unique_ptr`, `std::lock_guard`.$content$, 2);

    -- 13.3
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '13.3. Move-семантика и rvalue', $content$До C++11 возврат больших объектов из функций приводил к дорогому копированию. Семантика перемещения позволяет "забрать" внутренности временного объекта вместо их копирования.
...
```$content$, 3);
    ...
    -- 13.4
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '13.4. Правило пяти (и Rule of Zero)', $content$Если ваш класс вручную управляет ресурсом (указателем), вам нужно определить:
...
**Rule of Zero**: Старайтесь проектировать классы так, чтобы они использовали готовые RAII-объекты (`std::vector`, `std::unique_ptr`). Тогда вам не придется писать ни один из этих методов — компилятор сделает всё правильно сам.$content$, 4);

    -- 13.5
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '13.5. Принципы SOLID', $content$Чтобы код не превратился в спагетти, следуйте принципам SOLID:
...
Знание паттернов (Singleton, Factory, Observer, Strategy и т.д.) помогает решать типовые задачи проектирования на "стандартном" языке архитекторов.$content$, 5);
