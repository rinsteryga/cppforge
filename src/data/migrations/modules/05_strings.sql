...
(v_mod_id, '5.1. Что такое C-строка?', $content$В C нет типа "строка". Есть только массивы символов `char`. 
...
**Опасность**: Если вы забудете про `\0`, функции будут читать память дальше конца строки. Это называется **Buffer Overread**.$content$, 1)
    RETURNING id INTO v_lesson_id;

    -- Lesson 5.2
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '5.2. Операции и Ввод', $content$Поскольку строка — это массив, её нельзя просто скопировать через `=` или сравнить через `==`. Вы лишь скопируете адреса, а не сам текст.

**Безопасный ввод**:
`scanf("%s", str)` — это **дыра в безопасности**. Если пользователь введет слово длиннее массива, программа "сломается" (Buffer Overflow).
Всегда ограничивайте длину: `scanf("%19s", str);` (для массива из 20 байт, 1 байт всегда оставляем под `\0`).$content$, 2)
    RETURNING id INTO v_lesson_id;

    -- Lesson 5.3
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '5.3. Литералы и Память', $content$Строковый литерал `"Hello"` лежит в защищенной области памяти (ReadOnly).
...
```$content$, 3)
    ...
    -- Lesson 5.4
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '5.4. Библиотека <string.h>', $content$Основные функции:
- `strlen(s)`: длина.
...
**Внимание**: Вы сами должны гарантировать, что в `dest` достаточно места для `src`.$content$, 4)
    RETURNING id INTO v_lesson_id;
    ...
    VALUES (
        v_lesson_id, 
        'Собственный strlen', 
        $description$Напишите функцию int my_strlen(const char *str), которая вычисляет длину строки без использования <string.h>. В функции main реализован ввод строки.
...
Вывод: 5$description$, 
    ...
    -- Lesson 5.5
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
    (v_mod_id, '5.5. <ctype.h>', $content$Для работы с одиночными символами есть полезная библиотека <ctype.h>.
...
Нюанс: функции <ctype.h> могут вести себя странно при работе с кириллицей (и другими мультибайтными кодировками, такими как UTF-8), если не настроена локаль через setlocale(). В современном C++ для этого используют другие механизмы, а в C для UTF-8 лучше применять специализированные библиотеки, например, ICU.$content$, 5)
    RETURNING id INTO v_lesson_id;
    ...
    VALUES (
        ...
        $description$Напишите функцию void my_reverse(char *str), которая переворачивает строку на месте (in-place) без создания новых массивов. Использовать string.h запрещено! Функция main уже написана за вас.
