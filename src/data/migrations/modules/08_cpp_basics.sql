...
(v_mod_id, '8.1. g++/clang++ и первая программа на C++', $content$C++ — это компилируемый язык со строгой типизацией. Процесс превращения кода в программу состоит из 4 этапов:
...
```$content$, 1)
    ...
    -- Lesson 8.2
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '8.2. Новые базовые типы данных', $content$В C++ появились типы, делающие код безопаснее и выразительнее:
...
auto y = 5.5f; // float
```
Используйте `auto`, когда тип очевиден или слишком сложен (например, для итераторов), но не злоупотребляйте им.$content$, 2)
    RETURNING id INTO v_lesson_id;

    -- Lesson 8.3
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '8.3. Пространства имен и using', $content$Пространства имен (`namespace`) предотвращают коллизии имен в больших проектах.
...
```$content$, 3)
    ...
    -- Lesson 8.4
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '8.4. Ссылки — это не указатели', $content$Ссылка (`&`) — это "второе имя" для уже существующей переменной.
...
`void process(const LargeObject& obj)` — стандартный способ передачи данных в C++ (быстро как указатель, удобно как значение, безопасно из-за `const`).$content$, 4)
    RETURNING id INTO v_lesson_id;
    ...
    VALUES (
        ...
        $description$Напишите функцию `void square(int &num)`, которая возводит переданное число в квадрат. Используйте ссылку, чтобы изменить оригинал. Для ввода и вывода используйте `std::cin` и `std::cout` вместо `scanf` / `printf`.
...
Вывод: 25$description$, 
    ...
    -- Lesson 8.5
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '8.5. const и constexpr', $content$C++ помешан на константности. Это помогает компилятору оптимизировать код, а программисту — избегать ошибок.
...
```$content$, 5)
    ...
    -- Lesson 8.6
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '8.6. Операторы new и delete', $content$В C++ мы не используем `malloc`/`free`. Для работы с кучей (heap) есть операторы `new` и `delete`.
...
```
Неправильное использование `delete` вместо `delete[]` приведет к неопределенному поведению и утечкам памяти.$content$, 6)
    RETURNING id INTO v_lesson_id;
