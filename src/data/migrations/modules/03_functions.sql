...
(v_mod_id, '3.1. Объявление и Определение', 'Функция — это строительный блок программы. Она позволяет разбить сложную задачу на мелкие части.
...
```', 1)
    ...
    -- Lesson 3.2
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '3.2. Передача параметров и Стек', 'В C параметры передаются **только по значению**.
...
Локальные переменные живут на **стеке**. Как только функция завершается, её память освобождается автоматически.', 2)
    RETURNING id INTO v_lesson_id;
    ...
    -- Lesson 3.3
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '3.3. Рекурсия и Stack Overflow', 'Рекурсия — это когда функция вызывает саму себя.
...
**Опасность**: Каждый вызов занимает место на стеке. Если забыть условие выхода, программа упадет с ошибкой **Stack Overflow**.', 3)
    RETURNING id INTO v_lesson_id;
...
Вывод: 5', 
    ...
    -- Lesson 3.4
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '3.4. Препроцессор: #include и #define', 'До компиляции код проходит через препроцессор. Он не знает правил языка C, он просто манипулирует текстом.
...
```
**Золотое правило**: всегда берите аргументы макроса в скобки: `((x) * (x))`, но лучше используйте константы.', 4)
    RETURNING id INTO v_lesson_id;
