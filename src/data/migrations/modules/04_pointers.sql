...
Это означает, что массивы и указатели тесно связаны. Однако массив — это не указатель! У массива есть фиксированный размер на этапе компиляции, и вы не можете "перенаправить" имя массива на другой участок памяти.', 3)
    ...
    -- Lesson 4.4
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
    (v_mod_id, '4.4. Адресная арифметика', 'К указателям можно прибавлять и отнимать целые числа. 
...
Оператор индексации [] — это просто синтаксический сахар для адресной арифметики.', 4)
    RETURNING id INTO v_lesson_id;
...
При передаче двумерного массива в функцию компилятору необходимо знать длину всех измерений, кроме первого, чтобы правильно вычислять смещение:
  void process(int mat[][4], int rows) { ... }', 6)
    ...
    -- Lesson 4.7
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
    (v_mod_id, '4.7. Углубление в привычные scanf и printf', 'Теперь, когда мы знаем про указатели, функция scanf становится полностью понятной.
...
Функция printf в основном работает со значениями, поэтому мы передаем туда сами переменные: printf("%d", n). Но когда мы будем выводить строки (в следующем модуле), мы будем передавать указатели на их начало.', 7)
    RETURNING id INTO v_lesson_id;
