...
(v_mod_id, '12.1. Контейнеры', $content$STL предоставляет богатый набор структур данных. Основные категории:
...
```$content$, 1)
    ...
    VALUES (
        ...
        $description$Напишите программу, которая считывает число N (количество элементов), затем считывает N целых чисел в `std::vector`. После этого найдите и выведите сумму всех элементов вектора.
...
Вывод: 60$description$, 
    ...
    -- 12.2
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '12.2. Итераторы', $content$Итераторы — это мост между контейнерами и алгоритмами. Они работают как обобщенные указатели. Любой контейнер STL предоставляет метод `begin()` (указывает на первый элемент) и `end()` (указывает на позицию **ЗА** последним элементом). Этот интервал `[begin, end)` называется полуоткрытым.
...
```
Типы итераторов бывают разными: от простых (Forward) до продвинутых (Random Access, как у вектора), которые позволяют прыгать сразу на N элементов вперед: `it + 5`.$content$, 2);

    -- 12.3
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '12.3. Алгоритмы', $content$Философия STL: "Алгоритмы отделены от данных". Библиотека `<algorithm>` содержит более 100 функций, которые принимают итераторы.
Основные категории:
...
```$content$, 3)
    ...
    VALUES (
        v_lesson_id, 
        'Сортировка вектора', 
        $description$Считайте число N, затем N целых чисел в `std::vector`. Отсортируйте вектор по возрастанию, используя `std::sort` из `<algorithm>`, и выведите элементы через пробел.
...
Вывод: 1 2 4 5$description$, 
    ...
    -- 12.4
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '12.4. Ассоциативные контейнеры', $content$Когда нужен поиск быстрее, чем O(N), на сцену выходят ассоциативные контейнеры:
...
```$content$, 4);
    ...
    -- 12.5
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '12.5. fstream и потоки', $content$Работа с файлами в C++ строится на иерархии потоков. Основные классы: `ifstream` (Input), `ofstream` (Output) и `fstream` (оба сразу).
...
```$content$, 5);
    ...
    -- 12.6
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '12.6. stringstream и string_view', $content$Эти два инструмента радикально упрощают работу со строками.
...
**Золотое правило**: если функция только читает строку, принимайте её по `std::string_view`.$content$, 6);

    -- 12.7
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '12.7. Типы-помощники: optional, variant, any', $content$Современный C++ (17) добавил типы, которые делают код чище и безопаснее:
...
- **`std::any`**: контейнер для значения абсолютно любого типа. Используется редко, когда нужна максимальная динамика.$content$, 7);
