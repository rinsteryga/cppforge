...
(v_mod_id, '2.1. Базовые типы данных', 'C — это язык с "ручным" управлением данными. Тип переменной определяет, как биты в памяти превращаются в числа.
...
**Важно**: Размер типов зависит от архитектуры процессора. Используйте оператор `sizeof(тип)`, чтобы узнать точный размер в байтах на вашей системе.', 1)
    RETURNING id INTO v_lesson_id;

    -- Lesson 2.2
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '2.2. Арифметика и логика', 'В C есть всё: от сложения до битовых сдвигов.
...
`if (42)` — выполнится. `if (0)` — нет.', 2)
    ...
    -- Lesson 2.3
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '2.3. Ввод данных: scanf', 'Функция `scanf` читает данные и записывает их по адресу переменной. Именно поэтому мы ставим оператор `&` (взять адрес).
...
**Профессиональный совет**: `scanf` возвращает количество успешно считанных элементов. Если вы ждете 2 числа, а пользователь ввел текст, `scanf` вернет значение меньше 2. Проверка этого значения — ключ к написанию надежных программ.', 3)
    RETURNING id INTO v_lesson_id;
...
Вывод: 12', 
    ...
    -- Lesson 2.4
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '2.4. Условный оператор и switch', 'Оператор `if-else` направляет поток выполнения программы. 
...
```
**Важно**: Если убрать `break`, после "Вт" программа напечатает и "Другой день"! Это называется fall-through.', 4)
    RETURNING id INTO v_lesson_id;
...
Используйте его для простых выражений. Не стоит злоупотреблять вложенными тернарными операторами — это сильно ухудшает читаемость кода.', 5)
    ...
    -- Lesson 2.6
    INSERT INTO lessons (module_id, title, content, order_index) VALUES
(v_mod_id, '2.6. Циклы: заставляем компьютер работать', 'Циклы — это сердце автоматизации. 
...
**Совет**: Избегайте бесконечных циклов (`while(1)`), если у вас нет четкого условия выхода внутри. Они могут "завесить" вашу программу или систему.', 6)
    RETURNING id INTO v_lesson_id;
