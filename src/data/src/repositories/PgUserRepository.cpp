...
namespace cppforge::repositories
{
    ...
    void PgUserRepository::loadUserAchievements(entities::User &user) const
    {
        ...
        if (query.exec())
        {
            ...
            {
                auto earnedAt =
                    std::chrono::system_clock::from_time_t(query.value("earned_at").toDateTime().toSecsSinceEpoch());
    ...
    void PgUserRepository::saveUserAchievements(const entities::User &user)
    {
        ...
        for (const auto &achievement : user.getAchievements())
        {
            ...
            insertQuery.bindValue(":earned_at", QDateTime::fromSecsSinceEpoch(
                                                    std::chrono::system_clock::to_time_t(achievement.getDateEarned())));
            insertQuery.exec();
        }
    }
